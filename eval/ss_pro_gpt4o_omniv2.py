...
def reformat_messages(parsed_content_list):
    ...
    for idx, element in enumerate(parsed_content_list):
        ...
        if element["type"] == "text":
            screen_info += (
                f"""<p id={idx} class="text" alt="{element['content']}"> </p>\n"""
            )
            # screen_info += f'ID: {idx}, Text: {element["content"]}\n'
        elif element["type"] == "icon":
            screen_info += (
                f"""<img id={idx} class="icon" alt="{element['content']}"> </img>\n"""
            )
...
class GPT4XModel:
    ...
    def ground_only_positive_phi35v(self, instruction, image):
        ...
        icon_id, bbox, click_point, response_text = get_pred_phi3v(
            prompt_origin,
            (base64_image, dino_labled_img),
            label_coordinates,
            id_key="Click ID",
